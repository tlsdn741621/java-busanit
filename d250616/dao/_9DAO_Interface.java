...
// DAO(Data Access Object) : DB 에 접근 하는 기능 모음집
// CRUD, 검색, 한명 조회, 필터 등
// 인터페이스를 만들기 -> 이걸 구현한느 클래스 형식 -> 느슨한 결합 작업
// 스프링의 기본 뼈대 작업 방식

// DTO(Data Transfer Object) : DB 에 전달하는 모델 클래스
// 데이터베이스, 회웑의 정보를 각각 따로 전달하기보다는, 모델 클래스에담아서, 인스턴스로 전달

public interface _9DAO_Interface {
    // 자바 특성, 클래스 끼리는 단일 상속 만 가능해서
    ...
    // 인터페이스를 이용 안 할 경우 -> 유지 보수시, 변경 및, 변경으로 인한 수정 해야 할 부분이 늘어남
    //
